...
const PricingTable = () => {
	const { data, loading } = useFetch<PricingTableAPI>('/pricing/pricingTable')
	if (loading) return <Loading />
	if (!data) return <></>

	return (
		<ColorBox color='transparent' border={{ show: true, size: '1px', radius: '6px' }}>
			<Table
				fields={data.header}
				...
				renderRow={(item) => (
					<TableRow key={item.feature} hover>
						...
						<TableCell align='center' value={<RowContent value={item.starter} />} />
						<TableCell align='center' value={<RowContent value={item.pro} />} />
						<TableCell align='center' value={<RowContent value={item.enterprise} />} />
					</TableRow>
				)}
			>
				<TableRow>
