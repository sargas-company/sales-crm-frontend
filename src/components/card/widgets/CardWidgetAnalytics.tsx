...
const CardWidgetAnalytics = () => {
	...
	const chartOptions: ApexOptions = {
		...
		grid: {
			show: true,
			strokeDashArray: 8,
			borderColor: theme.mode.name === 'dark' ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0,0,0,0.1)',
		},
	}
				...
				<Table
					...
					renderRow={(item) => (
						<TableRow compact={true}>
							<TableCell value={item.label} weight={600} border={{ show: false }} />
							<TableCell value={item.amount} align='right' border={{ show: false }} />
							<TableCell
								value={item.growth}
								weight={600}
								align='right'
							...
							/>
							<TableCell
								value={<ArrowIndicator status={item.status as any} color={true} />}
								align='right'
								border={{ show: false }}
							/>
						</TableRow>
