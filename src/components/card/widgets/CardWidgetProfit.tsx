...
const CardWidgetProfit = () => {
	...
	const chartOptions: { series: any; option: ApexOptions } = {
		option: {
			...
			grid: {
				strokeDashArray: 8,
				borderColor:
					theme.mode.name === 'dark' ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0,0,0,0.1)',
			},
		},
		series: [
			{
	...
	return (
		<Card>
			<GridInnerContainer noShifting={true} columnSpacing={0} rowSpacing={{ xs: 1.6, lg: 0 }}>
				<GridItem xs={12} sm={7}>
				...
				<GridItem xs={12} sm={5}>
					<Box display='flex' flexDirection='column'>
						...
						/>
						<Box display='flex' flexDirection='column' space={0.8} pt={10} mt={8}>
							{total.map((item) => (
								<CardItemAnalyticsWithIcon
									label={addCurrancy(formatNumber(item.amount))}
									icon={
										<CustomAvatar varient='rounded' skin='light' color={item.color}>
											{item.icon}
										</CustomAvatar>
									}
									tag={item.label}
							...
							<Box display='flex' flexDirection='column' justify='center' px={20}>
