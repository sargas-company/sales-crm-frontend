...
const sources = [
	formatData('Zipcar', 'Vuejs, React & HTML', 23232.32, 'info', <Diamond />, 'info', 54),
	formatData('BitBank', 'Sketch, Figma & XD', 5433, 'warning', <Bolt />, 'warning', 89),
	formatData('Aviot', 'HTML, Svelte, Nodejs', 43423, 'success', <AllInclusive />, 'success', 34),
]
const CardTotalEarning = () => {
	const items = sources.map((item, index) => (
		<CardItemWithIconProgress
			...
			progress={<ProgressLinear start={item.score} color={item.color} size='xsmall' />}
		/>
	))
	return (
		<Card>
