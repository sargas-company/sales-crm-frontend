	...
	return (
		<Card>
			...
			<Box px={20} display='flex' flexDirection='column'>
				{items.map((item) => (
					<Box display='flex' space={0.8} mb={28} align='center' key={item.title}>
						<Avatar src={item.profile} alt='meeting person' />
						<Box display='flex' align='center' justify='space-between' wrap='wrap' flex={1}>
							<Box display='flex' flexDirection='column' space={0.3}>
								...
								<Text
									...
									secondary
								>
									<CalendarTodayOutlined fontSize='inherit' style={{ marginRight: 5 }} />
									{item.date?.day} {item.date?.month} | {item.date?.from} - {item.date?.to}
								</Text>
							</Box>
							<Chip
								label={item.tag}
