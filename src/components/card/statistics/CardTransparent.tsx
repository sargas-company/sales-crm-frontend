	...
	return (
		<Div theme={theme.mode}>
			<Box display='flex' align='center' space={0.6}>
				<CustomAvatar varient='rounded' color={theme.mode.foreground} classes='card-icon'>
					{icon}
				</CustomAvatar>
				<Box display='flex' flexDirection='column'>
					<Text varient='caption' textTransform='capitalize' weight='medium' secondary>
						{title}
					</Text>
					<Box display='flex' align='center' space={0.5}>
						...
						<Box
							display='flex'
							align='center'
							className={status === 'inc' ? 'alert-success-text' : 'alert-error-text'}
						>
...
const Div = styled('div')`
	...
	padding: 20px;
	border: 1px solid ${({ theme }) => (theme.name === 'dark' ? '#ffffff24' : 'rgb(217 217 217)')};
	border-radius: 6px;

	& .card-icon {
		box-shadow: 0px 4px 12px 0px
			${({ theme }) => (theme.name === 'dark' ? 'rgb(28 28 28 / 38%)' : 'rgb(217 217 217)')};
	}
`
