import { CheckCircleOutline, ErrorOutline, HighlightOffOutlined } from '@mui/icons-material'
			...
			<Box display='flex' flexDirection='column' space={1}>
				...
				<Box style={{ transform: 'translateX(-10px)' }}>
					...
					{error && typeof confirm === 'boolean' ? (
						<Text varient='caption' color='error' styles={{ marginLeft: 36 }} paragraph>
...
const Confirmation = forwardRef<RootModalToggleHandler>((props, ref) => {
	...
	const { show: showConfirmSuccess, toggleModal: toggleConfirmSuccess } = useModal()
	const { show: showConfirmCancel, toggleModal: toggleConfirmCancel } = useModal()
