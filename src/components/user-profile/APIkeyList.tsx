...
const APIkeyList = () => {
	const { data, loading } = useFetch<Array<APIkeyListAPI>>('/user_profile/apiKey')
	return (
		<Card padding='1.6rem'>
			<Box display='flex' flexDirection='column' space={1.6}>
				...
				{loading ? <Loading /> : data?.map((item) => <ListItem {...item} key={item.id} />)}
			</Box>
		</Card>
	)
}
...
const ListItem: FC<APIkeyListAPI> = ({ id, apiName, permission, assignedkey, createdDate }) => (
	<ColorBox
	...
	>
		...
		<Box display='flex' align='center' space={0.8}>
			...
			<IconButton varient='text' color='lightgray' fontSize={18} size={36} contentOpacity={5}>
				<ContentCopy />
			</IconButton>
		</Box>
		<Text varient='body1' paragraph secondary>
