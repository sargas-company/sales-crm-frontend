import { PersonAddAltOutlined, SentimentSatisfiedAltOutlined } from '@mui/icons-material'
...
const ConnectionList = () => {
	const { data, loading } = useFetch<Array<ConnectionsAPI>>('/user_profile/connections')
			...
			<Box key={cn.id} display='flex' align='center' justify='space-between'>
				...
				/>
				{
					<IconButton
						...
						fontSize={21}
					>
						{cn.isFriend ? <SentimentSatisfiedAltOutlined /> : <PersonAddAltOutlined />}
					</IconButton>
				}
			</Box>
		)
	...
	return (
		<Card pb='1.2rem'>
			...
			<ViewAllText text='connections' url='./' style={{ marginTop: 12, display: 'block' }} />
		</Card>
	)
}
