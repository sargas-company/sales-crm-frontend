...
import { selectEmail, setCurrentOpenMail, starredEmail } from '../../features/emails/creator/'
...
const selectMailItem = (mails: Mail[], mailId: MailId) => mails.find((mail) => mail?.id === mailId)
...
const EmailListItem: FC<{ mailId: MailId }> = ({ mailId }) => {
	...
	const mailItem = useAppSelector((state) => selectMailItem(state.email.mails!, mailId))
	...
	return (
		...
		>
			...
			<Box display='flex' align='center'>
				<Box display='flex' align='center' className='item-meta-info' space={0.6}>
...
const StyledEmailListItem = styled(Box)`
	...
	border-bottom-color: ${({ theme }) =>
		theme.mode.name === 'dark' ? `rgba(255, 255, 255, 10%)` : `rgba(0, 0, 0, 10%)`};
	cursor: pointer;
	transition: 200ms;

	&.unreaded-email {
		background: ${({ theme }) =>
			theme.mode.name === 'dark' ? 'rgba(255, 255, 255, 4%)' : `rgba(0, 0, 0, 3%)`};
	}
	&:hover {
		top: -3px;
		z-index: 2;
