			...
			<Box display='flex' py={12} px={20}>
				<Box display='flex' align='center' flex={1}>
					<Checkbox
						...
						checkedIcon={selectedType !== 1 ? <IndeterminateCheckBox /> : <CheckBox />}
					...
					>
						<ToolbarButton icon={<DeleteOutline />} handleClick={onRemoveMultiple} />
						<ToolbarButton icon={<MailOutlineRounded />} handleClick={onReadMultiple} />

						<Folders onMoveToSpam={onMoveToSpam} onRemoveMultiple={onRemoveMultiple} />
						<LablesDropdown onLabelClick={onLabelAdd} />
					</Box>
				</Box>
				<Box display='flex' align='center'>
					<ToolbarButton icon={<Refresh />} handleClick={() => console.log('re')} />
					<ToolbarButton icon={<MoreVert />} handleClick={() => console.log('re')} />
				</Box>
			</Box>
		</StyledToolbar>
	)
