...
const DataGridDropDownMain: FC<PropsType> = ({ fieldId, label }) => {
	...
	const drpList = [
		formatList(2, 'Sort By ASC', () => option?.sortByAsc({ fieldName: fieldId, by: 'asc' })),
		formatList(3, 'Sort By DESC', () => option?.sortByDesc({ fieldName: fieldId, by: 'desc' })),
	...
	return <Dropdown data={drpList} visible={false} dropdownId={fieldId} label={label} />
}
