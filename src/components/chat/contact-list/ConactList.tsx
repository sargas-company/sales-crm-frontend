...
import { Contacts } from '../../../store/chats/definition'
...
const ContactList: FC<Props> = ({ searchKey, chatContact }) => {
	...
	if (searchKey && filterChatId.length === 0) return <NotFound msg='no contacts found' />

	const renderChastList = filterChatId.map((chat) => (
		<ContactItem key={chat.profile.uid} uid={chat.profile.uid} onSelectChat={handleSelectChat} />
	))
