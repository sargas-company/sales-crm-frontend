...
type Operations =
	...
	| OperationList.EQUALS
	| OperationList.ENDSWITH
	| OperationList.STARTWITH
	| OperationList.EMPTY
	| OperationList.NOTEMPTY
...
const filterList = (items: {}[], fieldName: string) => {
	return (operator: Operations, value?: string | number) => {
		...
		switch (operator) {
			case 'contains':
				const filterByContains = filter((item) =>
					normalizFieldValue(item[fieldId]).includes(normalizFieldValue(value!))
				)
			...
			case 'equals':
				const filterByEqual = filter(
					(item) => normalizFieldValue(item[fieldId]) === normalizFieldValue(value!)
				)
				return filterByEqual

			case 'notEmpty':
				const filterByNotEmpty = filter((item) =>
					item[fieldId] === '' || item[fieldId] === null || item[fieldId] === undefined
						? false
						: true
				)
			...
			case 'startWith':
				const filterByStartWith = filter((item) =>
					normalizFieldValue(item[fieldId]).startsWith(normalizFieldValue(value!))
				)
				return filterByStartWith

			case 'endsWith':
				const filterByEndsWith = filter((item) =>
					normalizFieldValue(item[fieldId]).endsWith(normalizFieldValue(value!))
				)
