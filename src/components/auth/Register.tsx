...
import { Button, Checkbox, IconButton, Text, TextField } from '../../ui'
...
const Register = ({ onSubmit }: SubmitForm) => {
	...
	const clearError = useCallback(
		(key: keyof Errors) => setErrors((prevState) => ({ ...prevState, [key]: '' })),
		[]
	)
	...
	return (
		<AuthFormContainer>
			<Form autcomplete='OFF' onSubmit={handleSubmit} preventDefault>
				...
				/>
				<Box display='flex' flexDirection='column' width='100%' space={1} mb={28}>
					<Box display='block'>
						<TextField
							type='text'
							name='username'
