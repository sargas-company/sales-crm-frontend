...
const NewItem: FC<ItemDetail> = (props) => {
	const { id, cost, description, discount, hours, title, handleChange, handleRemoveItem } = props
	return (
		<ColorBox
			display='flex'
			pl={16}
					...
					<Box display='flex' flexDirection='column' space={0.4} pt={6} pr={20}>
						<Box display='flex' flexDirection='column' mb={8}>
							...
							<Select sizes='small' defaultValue={title}>
								...
								<SelectItem label='App Customization' value='app customization' />
				...
				<GridItem xs={12} md={2}>
					<Box display='flex' flexDirection='column' pt={6}>
						...
						<TextField
							...
							maxWidth='220px'
						/>
						<Text varient='body2' weight='medium' styles={{ marginTop: 16 }} secondary>
							Discount: {discount}%
						</Text>
					</Box>
				</GridItem>
