		...
		<Box display='flex' flexDirection='column' space={0.8} px={20}>
			...
			{selected && (
				<Box my={12} display='flex' flexDirection='column' space={0.2}>
					<Text varient='body2'>{leadTo[selected as keyof LeadKey].name}</Text>
					<Text varient='body2'>{leadTo[selected as keyof LeadKey].address}</Text>
					<Text varient='body2'>{leadTo[selected as keyof LeadKey].phone}</Text>
					<Text varient='body2'>{leadTo[selected as keyof LeadKey].mail}</Text>
				</Box>
			)}
		</Box>
	)
