...
import { useGetClientRequestByIdQuery, useUpdateClientRequestMutation } from '../../../store/clientRequests/clientRequestsApi'
import { useToast } from '../../../context/toast/ToastContext'
import parseServerError from '../../../utils/parseServerError'
import type { ClientRequestItem, ClientRequestStatus } from '../../../store/clientRequests/types/definition'
...
const ClientRequestFormInner = ({ id, initialData }: { id: string; initialData: ClientRequestItem }) => {
    ...
    <Card py="2rem" px="2rem">
      <Box style={{ maxWidth: 720, margin: '0 auto' }}>
        <Box mb={5}>
          ...
          <Box mt={1}>
            <Text varient="body2" secondary>Update the request details below</Text>
          </Box>
        </Box>
              ...
              <GridInnerContainer spacing={2}>
                ...
                <GridItem xs={12} md={6}>
                  <Box display="flex" flexDirection="column" space={1}>
                    <Text varient="body2" weight="medium">Name</Text>
                    <TextField
                      name="name"
                ...
                <GridItem xs={12} md={6}>
                  <Box display="flex" flexDirection="column" space={1}>
                    <Text varient="body2" weight="medium">Company</Text>
                    <TextField
                      name="company"
                ...
                <GridItem xs={12} md={6}>
                  <Box display="flex" flexDirection="column" space={1}>
                    <Text varient="body2" weight="medium">Email</Text>
                    <TextField
                      name="email"
                      type="email"
                ...
                <GridItem xs={12} md={6}>
                  <Box display="flex" flexDirection="column" space={1}>
                    <Text varient="body2" weight="medium">Phone</Text>
                    <TextField
                      name="phone"
...
const ClientRequestForm = ({ id }: { id: string }) => {
  ...
  if (!data) {
    return (
      <Card py="2rem" px="2rem">
        <Box style={{ maxWidth: 720, margin: '0 auto' }}>
          <Text varient="body2" secondary>Client request not found</Text>
        </Box>
      </Card>
    )
  }
