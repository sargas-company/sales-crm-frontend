		...
		<Card padding='1rem'>
			<Box display='flex' flexDirection='column' space={0.8}>
				...
				<Box display='flex' flexDirection='column' my={6}>
					...
					<Box display='flex' align='center' justify='space-between' mt={6}>
						...
						<Box display='flex'>
							<IconButton varient='text' contentOpacity={7} onClick={toggleModal}>
			...
			{show ? (
				<Modal handleOutClick={toggleModal}>
					<ModalContent value='+977-982-4965817' handleCloseModal={toggleModal} />
				</Modal>
			) : (
				''
			)}
...
const ModalContent = ({
	...
	<ModalContentLayout>
		...
		>
			...
			<Box display='flex' flexDirection='column' width='100%' px={8} pb={8} space={0.6}>
				<Text heading='h6'>Verify Your Mobile Number for SMS</Text>
				<Text varient='body2' styles={{ marginBottom: 8 }} secondary>
					Enter your mobile number with country code and we send you a verification code.
				</Text>
				<TextField
					type='text'
					name='contactNo'
