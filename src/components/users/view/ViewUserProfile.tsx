...
const ViewUserProfile: FC<UsersDataType> = (props) => {
	const { avatar, avatarColor, username, fullName, role, email, contact, status, country } = props
	return (
...
const ProjectsCompletetion = () => {
	return (
		<Box display='flex' align='center' justify='space-between' padding={1} my={28}>
			<ProjectItem title='Task Done' total='1.23k' icon={<Done />} />
			<ProjectItem title='Project Done' total='534' icon={<BusinessCenterOutlined />} />
		</Box>
	)
}
const ProjectItem = ({
