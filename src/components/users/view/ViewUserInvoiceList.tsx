...
const UserInvoiceList = () => {
	const [invoiceList, setInvoiceList] = useState(useAppSelector(selectInvoiceList))
			...
			<DataGrid
				...
				renderGridData={(item, column) => (
					<>
						...
						<DataGridCell width={column['total'].width} value={`$${item.total}`} />
						<DataGridCell width={column['issuedDate'].width} value={item.createdAt} />
