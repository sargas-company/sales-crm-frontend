...
const TableCell: FC<PropsType> = (props) => {
	...
	const { value, label, align, border, compact, as, weight, style, fontSize } = props
	return (
		<Cell
...
const color = (mode: 'dark' | 'light') => (mode === 'dark' ? 'rgb(84 84 84)' : 'rgb(221 221 221)')
...
const Cell = styled('td')<StyledProps>`
	...
	${({ border, theme }) =>
        ...
        ${border.right ? `border-right: 1px solid ${color(theme.mode.name)};` : ''}
        ${border.bottom ? `border-bottom: 1px solid ${color(theme.mode.name)};` : ''}
