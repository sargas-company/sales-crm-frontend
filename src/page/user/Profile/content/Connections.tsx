import { HowToRegOutlined, MailOutline, PersonAddOutlined } from '@mui/icons-material'
...
const Connections = () => {
	const { data, loading, error } = useFetch<ConnectionAPI[]>('/user_profile/connections')
			...
			<Card padding='1rem'>
				...
				>
					...
					<Box display='flex' align='center' space={1}>
						...
						<IconButton varient='outlined' roundness='rounded' size={36} fontSize={21}>
							<MailOutline />
						</IconButton>
					</Box>
	...
	return <GridInnerContainer spacing={1.6}>{renderConnectionList}</GridInnerContainer>
}
