...
const Project = () => {
	const { data, loading, error } = useFetch<Array<ProjectAPI>>('/user_profile/projects')
		...
		<GridItem xs={12} md={6} lg={4} key={pr.id}>
			<Card py='1.6rem'>
				<Box display='flex' flexDirection='column' space={0.8} px={20}>
					<ProjectHeading title={pr.title} client={pr.client} avatar={pr.avatar} id={pr.id} />
					...
					<ProjectDescription description={pr.description} upto={100} words={50} />
				</Box>
				...
				<Box display='flex' flexDirection='column' space={0.8} px={20}>
					<Box display='flex' justify='space-between' align='center'>
						...
						<Chip label={`${pr.daysLeft} Days left`} size='small' skin='light' />
					</Box>
					<ProjectProgress totalTask={pr.totalTask} completedTask={pr.completedTask} />
					<Box display='flex' justify='space-between' align='center'>
						...
						<Text
							...
							secondary
						>
							<ChatBubbleOutlineRounded style={{ fontSize: 21, marginRight: 4 }} />
							{pr.comments}
						</Text>
					</Box>
				</Box>
	...
	return <GridInnerContainer spacing={1.4}>{renderProjectList}</GridInnerContainer>
}
