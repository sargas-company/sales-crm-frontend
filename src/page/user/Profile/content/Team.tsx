...
const Team = () => {
	const { data, loading, error } = useFetch<Array<TeamAPI>>('/user_profile/teams/')
			...
			<GridItem key={index} xs={12} md={6} lg={4}>
				<Card padding='1.2rem'>
					<Box display='flex' flexDirection='column' space={1.2}>
						...
						<ItemDescription description={tm.description} upto={150} words={70} />
	...
	return <GridInnerContainer spacing={1.6}>{renderTeamList}</GridInnerContainer>
}
export default Team
