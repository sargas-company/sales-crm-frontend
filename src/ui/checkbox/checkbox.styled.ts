...
const backgroundStyle = (themeColor: string, color?: string) =>
	color
...
export const LabelText = styled.span<LabelTextProps>`
	color: ${({ theme, labelColor, checked, checkedLabelColor }) =>
		checked
			? alertColor[checkedLabelColor as keyof Colors]
			: labelColor
				? alertColor[labelColor as keyof Colors]
				: theme.mode.name === 'dark'
					? '#A8A4CE'
					: '#14274E'};
	cursor: pointer;
