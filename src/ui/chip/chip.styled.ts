...
const ChipWrapper = styled('span')<ChipProps>`
	...
	align-items: center;
	width: fit-content;
	background: ${({ theme, varient, color, skin }) =>
		varient === 'outlined' || skin
			? 'transparent'
			: varient === 'fill' && color
				? alertColor.hasOwnProperty(color)
					...
					: color
				: theme.color};
	color: ${({ color, varient, theme, skin }) =>
		varient === 'outlined' || skin === 'light'
			? color
				? alertColor.hasOwnProperty(color)
					? alertColor[color as keyof Colors]
					: color
	...
	max-height: 40px;
	border: 2px solid
		${({ varient, theme }) => (varient === 'outlined' ? theme.color : 'transparent')};
	cursor: default;
	overflow: hidden;
	z-index: 1;
