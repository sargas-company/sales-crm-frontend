...
export const InputWrapper = styled('div')<InputOptions>`
	...
	& .input-label {
		...
		background: ${({ theme, varient }) =>
			varient === 'filled' ? 'transparent' : theme.mode.foreground};
		${({ disable, theme, error }) =>
			(disable || error) &&
			`color: ${error ? alertColors('error') : disableColor(theme.mode.name)};`}
	...
	.input-button:focus-within + .floating-label {
		...
		color: ${({ theme, color, error }) =>
			error ? alertColors('error') : color ? alertColors(color) : theme.primaryColor.color};
		transform: scale(0.9);
		font-size: 13px;
		font-weight: 400;
	...
	& .floating-label-top {
		top: ${({ varient }) => (varient === 'filled' ? 8 : -10)}px;
		left: ${({ varient }) => (varient === 'standard' ? 0 : 0.5)}rem;
		color: ${({ theme, disable, error }) =>
			disable
				? disableColor(theme.mode.name)
				: error
					? alertColors('error')
	...
	.input-border {
		...
		background: ${({ theme, error }) =>
			error ? alertColors('error') : borderColor(theme.mode.name)};
	...
	.input-button:focus ~ .input-border-after,
	.active-border-transition:after {
...
export const Input = styled('input')<InputOptions>`
	...
	background: ${({ theme, varient }) =>
		varient === 'filled'
			...
			: 'transparent'};
	color: ${({ theme, disable }) =>
		disable ? disableColor(theme.mode.name) : theme.mode.textColor};
	border: 1.5px solid
		${({ theme, varient, error }) =>
			varient === 'filled' || varient === 'standard'
				...
				: error
					? alertColors('error')
					: borderColor(theme.mode.name)};
	padding: ${({ sizes }) => (sizes === 'small' ? 0.6 : 1)}rem 0.8rem;
	${({ varient }) =>
		varient === 'filled' &&
		`padding-bottom: 6px; padding-top: 26px; border-bottom-right-radius: 0px!important; border-bottom-left-radius: 0px!important;`}
	...
	&:focus,
	&:focus-visible {
		border-color: ${({ theme, varient }) =>
			varient === 'filled' || varient === 'standard' ? 'transparent' : theme.primaryColor.color};
		outline: ${({ theme, error }) => (error ? alertColors('error') : theme.primaryColor.color)}
			solid 2px;
