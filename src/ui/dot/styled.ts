...
const UiDot = styled('div')<DotProps>`
	position: relative;
	height: ${({ size }) => (size ? (size === 'small' ? 12 : size === 'large' && 24) : 15)}px;
	width: ${({ size }) => (size ? (size === 'small' ? 12 : size === 'large' && 24) : 15)}px;
	${({ outlined, outlineColor, theme }) =>
		outlined && `border: 2px solid ${outlineColor ? outlineColor : theme.primaryColor.color};`}
